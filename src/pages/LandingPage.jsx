...
const LandingPage = ({ onOpenInvitation }) => (
  ...
  >
    {/* Overlay untuk memperjelas konten */}
    <div className="absolute inset-0 bg-white/60 backdrop-blur-sm" />

    {/* Decorative Background */}
    <div className="absolute top-0 right-0 w-64 h-64 md:w-96 md:h-96 bg-rose-100/30 rounded-full blur-3xl translate-x-1/2 -translate-y-1/2" />
    <div className="absolute bottom-0 left-0 w-64 h-64 md:w-96 md:h-96 bg-pink-100/30 rounded-full blur-3xl -translate-x-1/2 translate-y-1/2" />
    ...
    <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4">
      ...
      >
        {/* Card Container */}
        <div className="backdrop-blur-md bg-white/50 p-6 sm:p-8 md:p-10 rounded-2xl border border-rose-100/50 shadow-xl">
          {/* Top Decorative Line */}
          ...
          >
            <div className="space-y-2">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif text-gray-800 leading-tight flex flex-wrap justify-center max-w-full break-words">
                <span className="truncate">{config.data.groomName}</span>
                <span className="text-rose-400 mx-2 sm:mx-3">&</span>
                <span className="truncate">{config.data.brideName}</span>
              </h1>
